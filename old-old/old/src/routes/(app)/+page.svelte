<script lang="ts">
	...
	onMount(async () => {
		...
		if (quizzes.length < 1) return goto('/new');
...
<div class="p-4 min-h-[calc(100vh-var(--spacing)*18)] flex flex-col gap-4">
	...
	{:else if quizzes.length < 1}
		<div class="flex-1 grid place-items-center">
			...
			>
				<span class="p-2 rounded-md bg-primary-50 text-primary-500">
					<SealQuestion class="size-6" />
				</span>

				<h1 class="font-medium text-xl">No resources yet</h1>
				<p>
					Start the learning process by creating a brand new resource, such as an assessment or a
					glossary.
				</p>

				<div class="h-4"></div>

				<Button size="lg" onclick={createQuiz} icon={FilePlus} label="New resource" />
			</div>
		</div>
	{:else}
		{@render header()}
...
{#snippet header()}
	<div class="flex mx-auto max-w-7xl gap-4 py-4 w-full">
		<span class="text-3xl font-medium">Materials</span>

		<span class="flex-1"></span>

		<Button href="/new" icon={FilePlus} label="New material" />
	</div>
{/snippet}
