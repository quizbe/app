<script lang="ts">
	...
	onMount(async () => {
		...
		if (!page.params.resourceid) return goto('/');
		const res = await loadQuiz(page.params.resourceid);
		if (!res) return goto('/');
		quiz.current = res;
		quiz.loading = false;
	});
...
>
	{#if quiz.editing < 0}
		<Icon class="h-8 text-primary-500" />
		...
		<Button icon={Gear} label="Settings" onclick={() => void (settings = true)} />

		<a href="/{page.params.resourceid}" class="flex items-center h-full">
			<Button
				icon={CaretLeft}
				label="Exit"
	...
	{:else}
		...
		{#if question}
			<Button icon={CaretLeft} onclick={() => void (quiz.editing = -1)} />
